...
function writeFeaturedItems() {
        /*
for(var i = (links.length-6); i < links.length; i++){
                document.write("<li><h3><a href = \"" + links[i].URL + "\">" + links[i].titleFull + " -" + links[i].authorFull + "<\/a><\/h3><\/li>");
        }
*/
}
...
function inPurify(element, pureList) {
	for (var i = 0; i < pureList.length; i++){
		if ((pureList[i].title === element.title) && (document.URL !== element.URL)) { //Replace with document.URL
